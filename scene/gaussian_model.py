"""GaussianModel with 4D deformation and alias-free 3D filtering.

Manages all Gaussian attributes (position, scale, rotation, SH, opacity),
the deformation network, and the 3D anti-aliasing filter from Mip-Splatting.
"""
...
class GaussianModel:
    ...
    def get_scaling_with_3D_filter(self):
        """Apply 3D smoothing filter to scales (Eq. 10 in the paper)."""
    ...
    def get_opacity_with_3D_filter(self):
        """Apply 3D smoothing filter to opacity (Eq. 10 in the paper)."""
    ...
    def compute_3D_filter(self, cameras):
        """Compute per-Gaussian 3D smoothing filter based on Nyquist sampling (Eq. 10)."""
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter):
        ...
        self.xyz_gradient_accum_abs[update_filter] += torch.norm(viewspace_point_tensor[update_filter, 2:], dim=-1, keepdim=True)
        self.xyz_gradient_accum_abs_max[update_filter] = torch.max(
            self.xyz_gradient_accum_abs_max[update_filter],
            torch.norm(viewspace_point_tensor[update_filter, 2:], dim=-1, keepdim=True))
    ...
    def _plane_regulation(self):
        ...
        for grids in multi_res_grids:
            time_grids = [] if len(grids) == 3 else [0, 1, 3]
    ...
    def _time_regulation(self):
        ...
        total = 0
        for grids in multi_res_grids:
            time_grids = [] if len(grids) == 3 else [2, 4, 5]
            for grid_id in time_grids:
                total += compute_plane_smoothness(grids[grid_id])
        return total

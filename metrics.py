"""SAGS Evaluation Script.

Computes PSNR, SSIM, LPIPS, FLIP, and RMSE metrics on rendered results.
"""
...
def cal_lpips(a, b, device="cuda", batch=2):
    ...
    for a_split, b_split in zip(a.split(split_size=batch, dim=0), b.split(split_size=batch, dim=0)):
        out = lpips_metric(a_split, b_split)
        lpips_all.append(out)
    lpips_all = torch.stack(lpips_all)
    return lpips_all.mean()
        ...
        for scene_dir in model_paths:
            ...
            for method in os.listdir(test_dir):
                ...
                for idx in tqdm(range(len(renders)), desc="Metric evaluation progress"):
                    render_img, gt, depth, gt_depth, mask = renders[idx], gts[idx], depths[idx], gt_depths[idx], masks[idx]

                    psnrs_star.append(psnr(render_img, gt, mask))

                    render_img = render_img * mask
                    gt = gt * mask
                    render_wmask.append(render_img)
                    gt_wmask.append(gt)
                    psnrs.append(psnr(render_img, gt))
                    ssims.append(ssim(render_img, gt))
                    lpipss.append(cal_lpips(render_img, gt))
                ...
                per_view_dict[scene_dir][method].update({
                    ...
                    "PSNR*": {name: p for p, name in zip(torch.tensor(psnrs_star).tolist(), image_names)},
